...
//
//
//@objc public protocol FCMChannelCurrentPollViewDelegate {
//    @objc optional func onBoundsChanged(_ currentPollView:FCMChannelCurrentPollView,currentPollHeight:CGFloat)
//    @objc optional func didFinishAnswerFlow(_ currentPollView:FCMChannelCurrentPollView,didReceiveLastMessage:Bool)
//    @objc optional func didFinishLoadFlow(_ currentPollView:FCMChannelCurrentPollView)
//}
//
//open class FCMChannelCurrentPollView: UITableViewCell, FCMChannelChoiceResponseDelegate, FCMChannelOpenFieldResponseDelegate, MDHTMLLabelDelegate {
//
...
//    open var delegates: [FCMChannelCurrentPollViewDelegate]?
//
...
//
//    var responses:[FCMChannelRulesetResponse] = []
//    var currentFlow:FCMChannelFlowDefinition?
//    var currentActionSet:FCMChannelFlowActionSet?
//    var currentRuleset:FCMChannelFlowRuleset?
//
//    open var viewController:UIViewController!
//    open var contact:FCMChannelContact!
//    var flowDefinition:FCMChannelFlowDefinition!
//    var flowRuleset:FCMChannelFlowRuleset?
//    var flowActionSet:FCMChannelFlowActionSet?
//
//    open var flowIsLoaded = false
//
//    var flowRule:FCMChannelFlowRule?
//    var response:String?
//
...
//    //MARK: Class Methods
//
//    open func setupLayoutParameters(_ contact:FCMChannelContact!,btNextTitle:String?,currentPollTitle:String?,switchLanguageTitle:String?,sendingAnswerMessage:String?,sendingAnswerError:String?,cancelTitle:String?,noAnswerTitle:String?,btNextColor:UIColor?,btNextTitleColor:UIColor?,removeHeader:Bool!) {
...
//        self.currentActionSet = FCMChannelFlowManager.getFlowActionSetByUuid(currentFlow!, destination: destination, currentActionSet: currentActionSet)
//        self.currentRuleset = FCMChannelFlowManager.getRulesetForAction(currentFlow!, actionSet: currentActionSet)
//    }
//
...
//
//        if !FCMChannelFlowManager.isLastActionSet(currentActionSet) {
...
//
//            RapidProAPI.sendRulesetResponses(contact, responses: responses, completion: { () -> Void in
//                hud.hide(true)
//
...
////        RapidProAPI.getFlowRuns(contact, completion: { (flowRuns: [FCMChannelFlowRun]?) -> Void in
////            if let flowRuns = flowRuns {
////                if ((!flowRuns.isEmpty) && FCMChannelFlowManager.isFlowActive(flowRuns[0])) {
////                    RapidProAPI.getFlowDefinition(flowRuns[0].flow_uuid, completion: {
////                        (flowDefinition: FCMChannelFlowDefinition) -> Void in
...
//        let viewResponsesChildren = viewResponses.subviews as! [FCMChannelResponseView]
...
//    }
//
//    func getResponse() -> FCMChannelRulesetResponse {
//        return FCMChannelRulesetResponse(rule: self.flowRule!, response: self.response!)
//    }
//
//    fileprivate func getResponseFromRule(_ rule:FCMChannelFlowRule) -> String {
...
//    }
//
//    func setupData(_ flowDefinition: FCMChannelFlowDefinition, flowActionSet: FCMChannelFlowActionSet, flowRuleset:FCMChannelFlowRuleset?, contact:FCMChannelContact) {
//        self.flowRule = nil
//        self.response = nil
//
//        self.contact = contact
...
//    }
//
//    func setupDataWithNoAnswer(_ flowDefinition: FCMChannelFlowDefinition?, flowActionSet: FCMChannelFlowActionSet?, flowRuleset:FCMChannelFlowRuleset?, contact:FCMChannelContact?) {
//        self.flowRule = nil
//        self.response = nil
//
//        self.contact = contact
...
//    }
//
//    func getChoiceResponse(_ flowRule:FCMChannelFlowRule, frame:CGRect) -> FCMChannelResponseView {
//        let choiceResponseView = Bundle(for: FCMChannelCurrentPollView.self).loadNibNamed("FCMChannelChoiceResponseView", owner: self, options: nil)?[0] as! FCMChannelChoiceResponseView
//        choiceResponseView.frame = frame
//        choiceResponseView.delegate = self
//        return choiceResponseView
//    }
//
//    func getOpenFieldResponse(_ flowRule:FCMChannelFlowRule, frame:CGRect) -> FCMChannelResponseView {
//        let openFieldResponseView = Bundle(for: FCMChannelCurrentPollView.self).loadNibNamed("FCMChannelOpenFieldResponseView", owner: self, options: nil)?[0] as! FCMChannelOpenFieldResponseView
...
////            let languageDescription = FCMChannelCountry.getLanguageDescription(language, type: FCMChannelCountryCodeType.ISO3) ?? language
////
////            let switchLanguageAction = UIAlertAction(title: languageDescription, style: .Default, handler: {
////                (alert: UIAlertAction!) -> Void in
////
////                FCMChannelSettings.savePreferedLanguage(language)
////
////                self.selectedLanguage = language
////                self.setupQuestionTitle()
////                for responseView in self.viewResponses.subviews as! [FCMChannelResponseView] {
...
//        self.lbQuestion.htmlText = FCMChannelFlowManager.translateFields(contact, message: (flowActionSet?.actions?[0].message == nil || flowActionSet?.actions?[0].message.count == 0 ? noAnswerTitle : flowActionSet?.actions?[0].message[getSelectedLanguage()])!)
...
//            if !FCMChannelFlowManager.hasRecursiveDestination(flowDefinition, ruleSet: flowRuleset, rule: flowRule) {
//
...
//                var responseView:FCMChannelResponseView?
//
